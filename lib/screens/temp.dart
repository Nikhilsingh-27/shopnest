...
class _CheckoutScreenState extends State<CheckoutScreen> {
  String? orderId; // ✅ store backend order_id
  ...
  void openRazorpay(String razorpayOrderId, double totalAmount) {
    var options = {
      ...
      'order_id': razorpayOrderId, // ✅ VERY IMPORTANT
      'name': 'ShopNest',
  ...
  Future<void> checkoutApi() async {
    try {
      ...
      var response = await GetConnect().post(
        "https://yourdomain.com/api/orders/checkout",
        body,
      );
      ...
      if (response.statusCode == 201) {
        ...
        if (paymentMethod == "cod") {
          ...
          Get.offAllNamed("/orders"); // or success page
          return;
        }

        String razorpayOrderId = data["razorpay_order"]["id"];
        double total = data["total"];

        openRazorpay(razorpayOrderId, total);
      } else {
  ...
  void handlePaymentSuccess(PaymentSuccessResponse response) async {
    try {
      var res = await GetConnect().post(
        "https://yourdomain.com/api/orders/verify-payment",
        {
          "order_id": orderId, // store this from checkout response
          ...
          Row(
            ...
            children: [
              ...
              ElevatedButton.icon(
                ...
                onPressed: () {
                  if (validate()) {
                    checkoutApi(); // ✅ single entry point
                  }
                },
